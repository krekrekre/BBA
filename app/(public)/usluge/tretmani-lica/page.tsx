...
export const metadata: Metadata = {
  title: "Tretmani lica",
  description:
    "Profesionalni tretmani lica - čišćenje, hidratacija, anti-aging i specijalni tretmani u našem salonu.",
};

export default async function TretmaniLicaPage() {
  const supabase = await createServerSupabaseClient();

  const { data: services } = await supabase
    .from("services")
    .select("*")
    .eq("category", "tretmani-lica")
    .eq("is_active", true)
    .order("order_index")
    .order("title_rs");
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      ...
      <p className="text-text-dark/80 mb-12 max-w-2xl">
        Profesionalna nega lica – čišćenje, hidratacija, anti-aging i specijalni
        tretmani prilagođeni vašoj koži.
      </p>
