...
export const metadata: Metadata = {
  title: "Epilacija",
  description:
    "Trajna epilacija - rešite se neželjenih dlaka na dugi rok. Profesionalna usluga u našem salonu.",
};

export default async function EpilacijaPage() {
  const supabase = await createServerSupabaseClient();

  const { data: services } = await supabase
    .from("services")
    .select("*")
    .eq("category", "epilacija")
    .eq("is_active", true)
    .order("order_index")
    .order("title_rs");
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      ...
      <p className="text-text-dark/80 mb-12 max-w-2xl">
        Trajna epilacija – rešite se neželjenih dlaka na dugi rok. Profesionalna
        usluga uz savremenu opremu.
      </p>
